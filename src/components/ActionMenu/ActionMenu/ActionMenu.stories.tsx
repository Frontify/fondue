/* (c) Copyright Frontify Ltd., all rights reserved. */

import { MenuItemContentSize, MenuItemStyle, SelectionIndicatorIcon } from '@components/MenuItem';
import { action } from '@storybook/addon-actions';
import { Meta, StoryFn } from '@storybook/react';
import React from 'react';
import { ActionMenu as ActionMenuComponent, ActionMenuItemType, ActionMenuProps } from './ActionMenu';
import IconMusicNote from '@foundation/Icon/Generated/IconMusicNote';

export default {
    title: 'Components/Action Menu',
    component: ActionMenuComponent,
    argTypes: {
        onClick: { action: 'onClick' },
    },
} as Meta;

export const ActionMenu: StoryFn<ActionMenuProps & { onClick: () => void }> = (args) => (
    <ActionMenuComponent {...args} />
);

const baseMenuItemProps: Pick<ActionMenuItemType, 'onClick' | 'decorator' | 'selectionIndicator'> = {
    onClick: action('click'),
    decorator: <IconMusicNote />,
    selectionIndicator: SelectionIndicatorIcon.CaretRight,
};

ActionMenu.args = {
    menuBlocks: [
        {
            id: 'block1',
            ariaLabel: 'First section',
            menuItems: [
                {
                    ...baseMenuItemProps,
                    id: '1',
                    title: 'Small item with icon',
                    size: MenuItemContentSize.Small,
                    onClick: (value: unknown) => {
                        action('switch toggled')(value);
                    },
                    type: 'switch',
                    initialValue: false,
                },
                {
                    ...baseMenuItemProps,
                    id: '2',
                    title: 'Small item with icon danger',
                    size: MenuItemContentSize.Small,
                    style: MenuItemStyle.Danger,
                },
                {
                    ...baseMenuItemProps,
                    id: '3',
                    title: 'Small item with icon warning',
                    size: MenuItemContentSize.Small,
                    style: MenuItemStyle.Warning,
                },
                {
                    ...baseMenuItemProps,
                    id: '4',
                    title: 'Small item with icon disabled',
                    size: MenuItemContentSize.Small,
                    disabled: true,
                    selectionIndicator: SelectionIndicatorIcon.None,
                },
                {
                    ...baseMenuItemProps,
                    id: '5',
                    title: 'Small item with icon danger disabled',
                    size: MenuItemContentSize.Small,
                    style: MenuItemStyle.Danger,
                    disabled: true,
                    selectionIndicator: SelectionIndicatorIcon.None,
                },
                {
                    ...baseMenuItemProps,
                    id: '6',
                    title: 'Small item with icon warning disabled',
                    size: MenuItemContentSize.Small,
                    style: MenuItemStyle.Warning,
                    disabled: true,
                    selectionIndicator: SelectionIndicatorIcon.None,
                },
            ],
        },
        {
            id: 'block2',
            ariaLabel: 'Second section',
            menuItems: [
                {
                    ...baseMenuItemProps,
                    id: '7',
                    title: 'Small item',
                    size: MenuItemContentSize.Small,
                    decorator: undefined,
                },
                {
                    ...baseMenuItemProps,
                    id: '8',
                    title: 'Small item danger',
                    size: MenuItemContentSize.Small,
                    style: MenuItemStyle.Danger,
                    decorator: undefined,
                },
                {
                    ...baseMenuItemProps,
                    id: '9',
                    title: 'Small item warning',
                    size: MenuItemContentSize.Small,
                    style: MenuItemStyle.Warning,
                    decorator: undefined,
                },
                {
                    ...baseMenuItemProps,
                    id: '10',
                    title: 'Small item disabled',
                    size: MenuItemContentSize.Small,
                    disabled: true,
                    decorator: undefined,
                },
                {
                    ...baseMenuItemProps,
                    id: '11',
                    title: 'Small item danger disabled',
                    style: MenuItemStyle.Danger,
                    size: MenuItemContentSize.Small,
                    disabled: true,
                    decorator: undefined,
                },
                {
                    ...baseMenuItemProps,
                    id: '12',
                    title: 'Small item warning disabled',
                    style: MenuItemStyle.Warning,
                    size: MenuItemContentSize.Small,
                    disabled: true,
                    decorator: undefined,
                },
            ],
        },
        {
            id: 'block3',
            ariaLabel: 'Third section',
            menuItems: [
                {
                    ...baseMenuItemProps,
                    id: '13',
                    title: 'Large item with icon',
                    subtitle: 'Subtitle',
                    size: MenuItemContentSize.Large,
                },
                {
                    ...baseMenuItemProps,
                    id: '14',
                    title: 'Large item with icon danger',
                    subtitle: 'Subtitle',
                    size: MenuItemContentSize.Large,
                    style: MenuItemStyle.Danger,
                },
                {
                    ...baseMenuItemProps,
                    id: '15',
                    title: 'Large item with icon warning',
                    subtitle: 'Subtitle',
                    size: MenuItemContentSize.Large,
                    style: MenuItemStyle.Warning,
                },
                {
                    ...baseMenuItemProps,
                    id: '16',
                    title: 'Large item with icon disabled',
                    subtitle: 'Subtitle',
                    size: MenuItemContentSize.Large,
                    disabled: true,
                    selectionIndicator: SelectionIndicatorIcon.None,
                },
                {
                    ...baseMenuItemProps,
                    id: '17',
                    title: 'Large item with icon danger disabled',
                    subtitle: 'Subtitle',
                    size: MenuItemContentSize.Large,
                    style: MenuItemStyle.Danger,
                    disabled: true,
                    selectionIndicator: SelectionIndicatorIcon.None,
                },
                {
                    ...baseMenuItemProps,
                    id: '18',
                    title: 'Large item with icon warning disabled',
                    subtitle: 'Subtitle',
                    size: MenuItemContentSize.Large,
                    style: MenuItemStyle.Warning,
                    disabled: true,
                    selectionIndicator: SelectionIndicatorIcon.None,
                },
            ],
        },
        {
            id: 'block4',
            ariaLabel: 'Fourth section',
            menuItems: [
                {
                    ...baseMenuItemProps,
                    id: '19',
                    title: 'Large item',
                    subtitle: 'Subtitle',
                    size: MenuItemContentSize.Large,
                    decorator: undefined,
                },
                {
                    ...baseMenuItemProps,
                    id: '20',
                    title: 'Large item danger',
                    subtitle: 'Subtitle',
                    size: MenuItemContentSize.Large,
                    style: MenuItemStyle.Danger,
                    decorator: undefined,
                },
                {
                    ...baseMenuItemProps,
                    id: '21',
                    title: 'Large item warning',
                    subtitle: 'Subtitle',
                    size: MenuItemContentSize.Large,
                    style: MenuItemStyle.Warning,

                    decorator: undefined,
                },
                {
                    ...baseMenuItemProps,
                    id: '22',
                    title: 'Large item disabled',
                    subtitle: 'Subtitle',
                    size: MenuItemContentSize.Large,
                    disabled: true,
                    decorator: undefined,
                    selectionIndicator: SelectionIndicatorIcon.None,
                },
                {
                    ...baseMenuItemProps,
                    id: '23',
                    title: 'Large item danger disabled',
                    subtitle: 'Subtitle',
                    size: MenuItemContentSize.Large,
                    style: MenuItemStyle.Danger,
                    disabled: true,
                    decorator: undefined,
                    selectionIndicator: SelectionIndicatorIcon.None,
                },
                {
                    ...baseMenuItemProps,
                    id: '24',
                    title: 'Large item warning disabled',
                    subtitle: 'Subtitle',
                    size: MenuItemContentSize.Large,
                    style: MenuItemStyle.Warning,
                    disabled: true,
                    decorator: undefined,
                    selectionIndicator: SelectionIndicatorIcon.None,
                },
            ],
        },
    ],
};
