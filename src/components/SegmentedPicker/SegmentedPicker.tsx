...
const SegmentedPickerItem = (props: SegmentedPickerItemProps) => {
    ...
    const getSegmentedPickerItemTestId = () => {
        switch (true) {
    ...
    return (
        <div key={item.id} className={merge(['tw-relative', isFocusVisible && FOCUS_STYLE])}>
            <div
                ...
                data-test-id={getSegmentedPickerItemTestId()}
                className={merge([
...
export const SegmentedPicker: FC<SegmentedPickerProps> = ({
    id: propId,
    items,
    activeItemId,
    onChange,
    ariaLabel = 'SegmentedPicker',
    disabled = false,
}) => {
...
SegmentedPicker.displayName = 'FondueSegmentedPicker';
