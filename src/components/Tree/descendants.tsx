...
import { createNamedContext, noop, usePrevious } from './utils/descendants';
...
export function useDescendant<DescendantType extends Descendant>(
    ...
    const { registerDescendant, unregisterDescendant, descendants } = useContext(context);
    ...
    const someDescendantsHaveChanged = descendants.some((descendant, index) => {
...
>(
    ...
    return function handleKeyDown(event: React.KeyboardEvent) {
        if (
            !['ArrowDown', 'ArrowUp', 'ArrowLeft', 'ArrowRight', 'PageUp', 'PageDown', 'Home', 'End'].includes(
                event.key,
            )
        ) {
            return;
        ...
        // descendants array.
        if (filter) {
            index = selectableDescendants.findIndex((descendant) => descendant.index === currentIndex);
        }
        ...
        function getNextOption() {
            ...
            if (atBottom) {
                ...
                if (getFirstOption()) {
                    ...
                    return selectableDescendants[(index + 1) % selectableDescendants.length];
                }
            }
            ...
            if (atTop) {
                ...
                if (getLastOption()) {
                    ...
                    return selectableDescendants[
                        (index - 1 + selectableDescendants.length) % selectableDescendants.length
                    ];
                }
            }
        }
