...
import { getNodeIdsInFlatArray, removeNodesFromFlatArray } from './nodes';
...
describe('nodes', () => {
    describe('removeNodesFromFlatArray', () => {
        it('returns empty array if no nodes and filters are passed', () => {
            const result = removeNodesFromFlatArray([], []);
            cy.wrap(result).should('deep.equal', []);
        });

        it('returns empty array if no nodes are passed', () => {
            const result = removeNodesFromFlatArray([], ['1']);
            cy.wrap(result).should('deep.equal', []);
        });

        it('should returns the same tree if no filter is passed', () => {
            const result = removeNodesFromFlatArray(FLAT_ARRAY_OF_NODES, []);
        ...
        it('removes nodes from tree', () => {
            ...
            const result = removeNodesFromFlatArray(FLAT_ARRAY_OF_NODES, nodeIdsToRemove);
