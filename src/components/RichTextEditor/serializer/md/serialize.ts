...
import { OptionType, serialize as remarkSerialize } from 'remark-slate';

const options = (editor: PlateEditor): OptionType => ({
    nodeTypes: {
...
/**
 * Serialize content from Slate format to Markdown
 */
export const serialize = (editor: PlateEditor, value: any[]) => {
    const serializeOptions = options(editor);
    return value.map((node) => remarkSerialize(node, serializeOptions)).join('\n');
};
