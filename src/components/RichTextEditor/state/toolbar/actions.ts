...
const isInlineStyleData = (data: ToolbarData): data is InlineStyleData => (data as InlineStyleData).style !== undefined;
...
export const updateInlineStyle = (_: ToolbarContext, { data }: DoneInvokeEvent<ToolbarData>): void => {
    if (isInlineStyleData(data)) {
        ...
        Transforms.setNodes(editor, { [style as string]: true }, { match: (n) => Text.isText(n), split: true });
    }
};
