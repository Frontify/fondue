...
import { DragProperties, ItemDragState, OrderableListItem, SortableList, SortableListProps } from '.';
...
const SortableListWithDefaultProps = ({
    renderContent = renderDefaultTestItems,
    items = testItems,
    dragDisabled = false,
    onMove = cy.stub(),
}: Partial<SortableListProps<TestItem>>) => (
    <SortableList renderContent={renderContent} dragDisabled={dragDisabled} items={items} onMove={onMove} />
);

describe('SortableList Component', () => {
    it('renders correct number of list items', () => {
        cy.mount(<SortableListWithDefaultProps />);
        cy.get(LIST_ID)
            .find(DRAGGABLE_ITEM)
    ...
    it('Does not crash if item list is empty', () => {
        cy.mount(<SortableListWithDefaultProps items={[]} />);
    ...
    it('Fires a move event when item is dropped over an insertion indicator', () => {
        ...
        cy.mount(<SortableListWithDefaultProps onMove={stubbedOnMove} />);
    ...
    it('Passes on correct drag state to each item', () => {
        ...
        cy.mount(<SortableListWithDefaultProps renderContent={renderWithDragState} />);
    ...
    it('Should disable drag events if dragDisabled prop is true, but maintain focus for navigation', () => {
        cy.mount(<SortableListWithDefaultProps dragDisabled={true} renderContent={renderWithFocusableItems} />);
        cy.get(DRAGGABLE_ITEM).each(($el) => {
