 ...
 *
 * - `editorEngine.ts` — `MountDocument` / `MountOptions` / `MountedDocument`,
 *   plus the `EditorHandle` that appears once a document is editable. Implemented by `adapters/prosemirror/`, driven by
 *   `ui/useEditorHandle.ts`. Neither side imports the other, so swapping the
 *   engine means writing one more `MountDocument`.
