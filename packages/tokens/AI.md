...
## Setup for AI-Assisted Figma→Code (CLAUDE.md snippet)

To enable AI tools (Claude, Cursor, etc.) to convert Figma designs into code using fondue tokens, paste this into your project's `CLAUDE.md` or `.cursorrules`:

```
## Design System: Fondue Tokens

This project uses @frontify/fondue-tokens for design tokens and Tailwind CSS.

When implementing UI from Figma designs:
- For Figma variable → Tailwind class mapping, read: node_modules/@frontify/fondue-tokens/dist/figma-mapping.md
- For full Tailwind class usage reference, read: node_modules/@frontify/fondue-tokens/AI.md
- Always prefer Fondue components (@frontify/fondue-components) over raw Tailwind when a component exists
- Always use composite typography classes (body-medium, heading-large) over individual font utilities
- Always pair colored backgrounds with their matching `on-` text color
- Check tailwind.config.ts for a class prefix (e.g., `tw-`) and prepend it to all classes
```
