...
const restrictedDropNodes: TreeChangeState = [
  {
    id: "images",
    name: "Images",
    isFolder: true,
    ...
    const renderTagged = (list: TreeChangeState): ReactNode =>
      list.map((node) =>
        node.isFolder ? (
          ...
          <Tree.Item key={node.id} id={node.id} label={node.name} tags={node.tags} />
        ),
      );
...
const renderNavigationNodes = (
  nodes: TreeChangeState,
  activeId: string,
...
const lazyChildrenByParent: Record<
  string,
...
export const LazyLoading: Story = {
  ...
  render: (args) => {
    ...
    const renderLazyNodes = (
      nodes: Array<{ id: string; name: string; isFolder: boolean }>,
    ): ReactNode =>
      nodes.map((node) => {
        if (!node.isFolder) {
