...
const customFontSizes = tailwindConfig.theme.fontSize;
const customBoxShadows = tailwindConfig.theme.boxShadow;
const customOutline = tailwindConfig.theme.extend.outline;
...
export default [
        ...
        .map((prefix) => {
            return Object.keys(customColors).map((colorName) => {
                if (typeof customColors[colorName] === "string") {
                    ...
                    return Object.keys(customColors[colorName]).map((colorVariation) => {
                        if (colorVariation === "DEFAULT") {
                            return `${prefix}-${colorName}`;
                        } else {
                            return `${prefix}-${colorName}-${colorVariation}`;
                        }
                    });
                }
            });
    ...
    // Custom box shadows
    ...Object.keys(customBoxShadows).map((boxShadow) => `shadow-${boxShadow}`),

    // Custom outlines
    ...["outline", "shadow-outline"]
        .map((prefix) => {
            return Object.keys(customOutline).map((outline) => `${prefix}-${outline}`);
        })
        .flat(Infinity),

    // Custom font sizes
    ...Object.keys(customFontSizes).map((fontSize) => `text-${fontSize}`),
];
